...
"""
    zeit.recommend.storm
    ~~~~~~~~~~~~~~~~~~~~

    This module has no description.

    Copyright: (c) 2011 by Nathan Marz
    Source: github.com/nathanmarz/storm-starter
    Note: Minor modifications made for PEP8 compliance.
    License: Apache 2.0, see below for more details.
...
class Tuple:
    ...
    def __repr__(self):
        tups = ((k, self.__dict__[k]) for k in sorted(self.__dict__.keys()))
        return '<%s%s>' % (self.__class__.__name__, ''.join(' %s=%r' % tups))
